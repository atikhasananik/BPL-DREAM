...
const Hero = ({setCoin}:IHeroProp) => {
    ...
    <div className={`my-15 p-8 bg-[#131313] flex gap-8 flex-col relative rounded-4xl items-center  container mx-auto overflow-hidden`}>
        <div className={`h-full w-full absolute top-0 left-0  -z-10`}>
            ...
            <button onClick={()=> setCoin(10000000)} className="btn border-0 outline-0 shadow-none rounded-lg  bg-yellow-300">Claim Free Credit</button>
            </div>
        </div>
    );
};
